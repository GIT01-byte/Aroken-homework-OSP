				...
				<ul>
					<li class="step" x-show="step === 1">
						...
						<div class="data-input csv_upload">
							<label><?php esc_html_e( 'Upload file (CSV, Excel, XML)', 'vg_sheet_editor' ); ?> </label>
						...
						<div class="data-input csv_url">
							<label><?php esc_html_e( 'File URL (CSV, Excel, XML)', 'vg_sheet_editor' ); ?> </label>								
						...
						<div class="data-input server_file">
							<label><?php esc_html_e( 'File location (CSV, Excel, XML)', 'vg_sheet_editor' ); ?> <a href="" data-wpse-tooltip="right" aria-label="
													<?php
						...
						<label class="" x-show="!isQuickSetupMode && !vgse_editor_settings.enable_simple_mode"><input type="checkbox" x-model="showAdvancedOptions" name="enable_advanced_source_options" class="toggle-advanced-options"> <?php esc_html_e( 'Show advanced options', 'vg_sheet_editor' ); ?></label>
						<div class="advanced-options" x-show="showAdvancedOptions && !isQuickSetupMode && !vgse_editor_settings.enable_simple_mode">
							<div class="field">
							...
							<div class="field">
								<label><input type="checkbox" x-model="taskArgs.auto_column_names" name="auto_column_names" class="auto_column_names" value="yes" /> <?php esc_html_e( 'Automatically add column names to the CSV/Excel/XML file?', 'vg_sheet_editor' ); ?></label>								
							</div>
						</div>
							<p x-show="!isQuickSetupMode && !vgse_editor_settings.enable_simple_mode">
							<?php
								/* translators: %s: Documentation link URL */
								echo wp_kses_post( sprintf( __( 'Tip. You can use the "export" tool to download a CSV and see the available columns and format.<br>You can read our <a href="%s" target="_blank">documentation here</a>.', 'vg_sheet_editor' ), esc_url( VGSE()->get_site_link( 'https://wpsheeteditor.com/blog/?s=import', 'importer-documentation' ) ) ) );
							?>
								</p>
					...
					<li class="map-columns step" x-show="step === 2">
						...
						echo wp_kses_post( sprintf( __( 'Important. We only detected one column in the CSV file. If this is incorrect, follow <a href="%s" target="_blank">these steps</a> to fix it', 'vg_sheet_editor' ), esc_url( 'https://wpsheeteditor.com/documentation/faq/#1572924330879-e05ed559-f740234' ) ) );
						?>
						</p>
						...
						<p x-show="!isQuickSetupMode && !vgse_editor_settings.enable_simple_mode"><?php esc_html_e( 'Tip. If you edited information from this site, you should import the columns edited and record_id. Don\'t import columns that weren\'t modified', 'vg_sheet_editor' ); ?></p>
									...
									<div class="map-template dynamic-column-template">
										<div class="dynamic-column-settings">
											...
											<div x-show="dynamicColumn.type === 'text_replace'">
												<template x-if="dynamicColumn.type === 'text_replace'">
													<div class="field-wrap">
														<label><?php esc_html_e( 'Source column', 'vg_sheet_editor' ); ?> <a href="#" data-wpse-tooltip="right" aria-label="<?php esc_attr_e( 'Select the column that will be the subject of the replacement, you can select dynamic columns too for advanced processing of dynamic values.', 'vg_sheet_editor' ); ?>">( ? )</a></label>
														<select :data-dynamic-combine-index="index" x-model="dynamicColumn.replace_source_column" x-init="initSelect2($el)" :data-full-model-path="'taskArgs.dynamic_columns['+index+'].replace_source_column'" class="select2">
															<option value="">--</option>
															<template x-for="csvHeader in getSourceColumnsForReplace(dynamicColumn.name)">
											...
											<template x-if="dynamicColumn.type === 'combine_columns'">
												<div>
													<div class="field-wrap">
						...
						<div class="field-find-existing-columns" x-show="taskArgs.writing_type && taskArgs.writing_type !== 'all_new'">						
							...
							<div class="field-wrapper">
								...
								<select x-model="taskArgs.existing_check_csv_field[0]" name="existing_check_csv_field[]" class="select2 existing-check-csv-field" x-init="initSelect2($el)">
									<option value="">- -</option>
									<template x-for="csvColumn in getSourceColumnsForReplace()">
					...
					<li class="preview-step step" x-show="step === 4 && !isQuickSetupMode">
						...
						<div class="field" x-show="!extra || !extra.scheduleEnabled">
							<label><?php esc_html_e( '3. Do you want to be able to run this import again with one click? Add a name for this import (optional)', 'vg_sheet_editor' ); ?></label>
							<br>
							<input type="text" x-model="taskArgs.name" name="name" class="name"/>
						</div>
						<label x-show="!vgse_editor_settings.enable_simple_mode" class=""><input type="checkbox" x-model="showAdvancedOptions" name="enable_advanced_source_options" class="toggle-advanced-options"> <?php esc_html_e( 'Show advanced options', 'vg_sheet_editor' ); ?></label>
						<div class="advanced-options" x-show="showAdvancedOptions && !vgse_editor_settings.enable_simple_mode">
							<div class="field">
