...
class VGSE_Provider_Custom_table extends VGSE_Provider_Abstract {
	...
	function _insert_row( $data ) {
		...
		if ( $context === 'insert' ) {
			...
			if ( isset( $new_data[ $primary_column_key ] ) ) {
				unset( $new_data[ $primary_column_key ] );
			}
			if ( isset( $new_data_format[ $primary_column_key ] ) ) {
				unset( $new_data_format[ $primary_column_key ] );
			}
	...
	function get_searchable_column_keys( $post_type ) {
		...
		return apply_filters( 'vg_sheet_editor/provider/custom_table/get_searchable_column_keys', $out, $all_columns, $post_type );
	}
		...
		foreach ( $keywords as $phrase ) {
			...
			foreach ( $words as $word ) {
				...
				foreach ( $searchable_columns as $column ) {
					$word_checks[]   = $column . " $operator %s ";
					$prepared_data[] = '%' . $wpdb->esc_like( $word ) . '%';
				}
				if ( ! empty( $word_checks ) ) {
					$glue            = ( $operator === 'NOT LIKE' ) ? ' AND ' : ' OR ';
					$phrase_checks[] = ' (' . implode( $glue, $word_checks ) . ') ';
				}
			}

			if ( ! empty( $phrase_checks ) ) {
				$checks[] = ' (' . implode( ' AND ', $phrase_checks ) . ') ';
			}
		}
