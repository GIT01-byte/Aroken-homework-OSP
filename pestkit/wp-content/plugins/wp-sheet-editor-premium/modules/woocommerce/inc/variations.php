...
if ( ! class_exists( 'WP_Sheet_Editor_WooCommerce_Variations' ) ) {
	...
	class WP_Sheet_Editor_WooCommerce_Variations {
		...
		function register_toolbar_items( $editor ) {
			...
			$editor->args['toolbars']->register_item(
				...
				array(
					...
					'content'               => esc_html__( 'Create variations', 'vg_sheet_editor' ),
					'id'                    => 'create-variation',
					'help_tooltip'          => esc_html__( 'Create and copy variations for variable products.', 'vg_sheet_editor' ),
			...
			$editor->args['toolbars']->register_item(
				...
				array(
					...
					'content'                    => esc_html__( 'Display variations', 'vg_sheet_editor' ),
					'id'                         => 'display-variations',
					'default_value'              => false,
					'extra_html_attributes'      => ':checked="$store.vgseFilters.activeFilters.wc_display_variations" @change="$store.vgseFilters.toggleYesCheckbox(\'wc_display_variations\')" :disabled="$store.vgseFilters.hasModifiedCells"',
					'container_extra_attributes' => 'x-data="vgseDisplayVariations"',
				),
				$this->post_type
			);
			...
			$editor->args['toolbars']->register_item(
				...
				array(
					'type'                  => 'button', // html | switch | button
					'content'               => esc_html__( 'Display all the variations', 'vg_sheet_editor' ),
					'allow_in_frontend'     => true,
					'parent'                => 'display_variations',
					'extra_html_attributes' => '@click.prevent="$store.vgseFilters.toggleYesCheckbox(\'wc_display_variations\')" :disabled="$store.vgseFilters.hasModifiedCells"',
				),
				$this->post_type
			);
			$editor->args['toolbars']->register_item(
				...
				array(
					'type'              => 'button', // html | switch | button
					'content'           => esc_html__( 'Display the variations of selected products', 'vg_sheet_editor' ),
					'allow_in_frontend' => true,
					'parent'            => 'display_variations',
				),
				$this->post_type
			...
			$editor->args['toolbars']->register_item(
				...
				array(
					'type'              => 'button', // html | switch | button
					'content'           => esc_html__( 'Only display the selected products with variations', 'vg_sheet_editor' ),
					'allow_in_frontend' => true,
					'parent'            => 'display_variations',
				),
				$this->post_type
			...
			$editor->args['toolbars']->register_item(
				'display_variables_with_variations',
				array(
					'type'                  => 'button', // html | switch | button
					'content'               => esc_html__( 'Only display variable products that have variations', 'vg_sheet_editor' ),
					'allow_in_frontend'     => true,
					'parent'                => 'display_variations',
					'extra_html_attributes' => '@click.prevent="searchVariableProductsWithVariations" :disabled="$store.vgseFilters.hasModifiedCells"',
				),
				$this->post_type
			);
			$editor->args['toolbars']->register_item(
				'display_variables_with_no_variations',
				array(
					'type'                  => 'button', // html | switch | button
					'content'               => esc_html__( 'Only display variable products that have no variations', 'vg_sheet_editor' ),
					'allow_in_frontend'     => true,
					'parent'                => 'display_variations',
					'extra_html_attributes' => '@click.prevent="searchVariableProductsWithNoVariations" :disabled="$store.vgseFilters.hasModifiedCells"',
				),
				$this->post_type
			);
		}
		...
		function maybe_lock_general_columns_in_variations( $posts, $wp_query, $spreadsheet_columns ) {
			...
			$locked_keys_in_general = array_unique( array_merge( $this->wc_variation_only_columns, array_intersect( array_diff( $whitelist_variations, $first_product_keys ), $columns_with_visibility ) ) );
		...
		function init() {
			...
			add_filter( 'vg_sheet_editor/filters/after_fields', array( $this, 'add_search_on_variations_field' ) );
		...
		function load_variations_per_product() {
			...
			if ( ! VGSE()->helpers->verify_nonce_from_request() || ! VGSE()->helpers->user_can_edit_post_type( $this->post_type ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'You dont have enough permissions to load rows.', 'vg_sheet_editor' ) ) );
			}
			if ( empty( $_REQUEST['product_ids'] ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'Please select a product.', 'vg_sheet_editor' ) ) );
			}
			$product_ids         = sanitize_text_field( wp_unslash( $_REQUEST['product_ids'] ) );
			$request_data        = array(
		...
		function allow_to_run_formula_on_variations_directly( $post_type ) {
			...
			?>
<template x-if="rowsSelectionType !== 'edit_plain_values'">
    <li class="run-on-variations-field">
        <select x-model="extra.wcRunBulkEditOnVariations" name="wpse_run_on_variations"
            x-init="$($el).trigger('change')">
            <option value="select_type">
                <?php esc_html_e( 'Select what type of products to edit', 'vg_sheet_editor' ); ?></option>
            <option value=""><?php esc_html_e( 'Edit only parent products', 'vg_sheet_editor' ); ?></option>
            <option value="on"><?php esc_html_e( 'Edit only variations', 'vg_sheet_editor' ); ?></option>
        </select>
    </li>
</template>
<?php
...
<div class="column-settings-field">
    <label><input value="yes" <?php checked( in_array( $column['key'], $whitelisted_columns, true ) ); ?>
            type="checkbox" name="column_settings[<?php echo esc_attr( $column['key'] ); ?>][allow_for_variations]">
        <?php esc_html_e( 'Allow to edit this column on variation rows?', 'vg_sheet_editor' ); ?></label>
    <p><?php esc_html_e( 'We automatically allow columns for variations if at least one variation is using that field.', 'vg_sheet_editor' ); ?>
    </p>
</div>
<?php
		}
		...
		function register_columns( $editor ) {
			...
			$editor->args['columns']->register_item(
				...
				array(
					'data_type'         => 'meta_data',
					'column_width'      => 175,
					'title'             => esc_html__( 'Variation description', 'woocommerce' ),
			...
			$editor->args['columns']->register_item(
				...
				array(
					...
					'data_type'             => 'post_data',
					'column_width'          => 140,
					'title'                 => esc_html__( 'Variation enabled?', 'woocommerce' ),
					'supports_formulas'     => true,
			...
			$editor->args['columns']->register_item(
				...
				array(
					...
					'column_width'                  => 210,
					'title'                         => esc_html__( 'Default attributes', 'woocommerce' ),
					'type'                          => 'handsontable',
					'edit_button_label'             => esc_html__( 'Default attributes', 'woocommerce' ),
					'edit_modal_id'                 => 'vgse-default-attributes',
					'edit_modal_title'              => esc_html__( 'Default attributes', 'woocommerce' ),
					/* translators: %s: Delimiter */
					...
					'prefetch_meta_key'             => '_default_attributes',
					'formatted'                     => array(
						'data'     => '_default_attributes',
						'renderer' => 'html',
					),
		...
		function get_default_attributes_for_cell( $value, $post, $column_key ) {
			...
			$raw_default = maybe_unserialize( VGSE()->helpers->get_current_provider()->get_item_meta( $post->ID, '_default_attributes', true ) );
		...
		function copy_variations( $data, $product_ids ) {
			if ( ! function_exists( 'WPSE_Queues_Obj' ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'Please update all the WP Sheet Editor plugins. This feature requires the latest version of all our plugins.', 'vg_sheet_editor' ) ) );
			}
			...
			if ( empty( $product_data['variations'] ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'The source product doesn\'t have variations.', 'vg_sheet_editor' ) ) );
			}
		...
		function process_copy_variations_queue() {
			...
			if ( ! WPSE_Queues_Obj()->queue_exists( $job_id ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'We don\'t have variations to copy.', 'vg_sheet_editor' ) ) );
			}

			$processed = $this->_process_copy_variations_queue( $job_id, $file_position );
			wp_send_json_success(
				array(
					'message'       => esc_html__( '{total_created} of {total} variations created.', 'vg_sheet_editor' ),
		...
		function create_variations_rows() {
			...
			$variation_manager_source = sanitize_text_field( wp_unslash( $_REQUEST['vgse_variation_manager_source'] ) );
			$variation_tool           = sanitize_text_field( wp_unslash( $_REQUEST['vgse_variation_tool'] ) );
			$page                     = isset( $_REQUEST['page'] ) ? (int) $_REQUEST['page'] : null;
			$job_id                   = sanitize_text_field( VGSE()->helpers->get_job_id_from_request() );
			$selected_product         = null;
			if ( isset( $_REQUEST['product'] ) ) {
				$selected_product = is_string( $_REQUEST['product'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['product'] ) ) : array_map( 'sanitize_text_field', $_REQUEST['product'] );
			}
			if ( $variation_tool === 'copy' ) {
				$data_from_request = array(
					'nonce'                      => sanitize_text_field( VGSE()->helpers->get_nonce_from_request() ),
					'copy_from_product'          => sanitize_text_field( wp_unslash( $_REQUEST['copy_from_product'] ) ),
			...
			if ( $variation_manager_source === 'individual' ) {
				...
				if ( $page > 1 && empty( $product_ids ) ) {
					...
					wp_send_json_success(
						array(
							/* translators: %s: Number of variations created */
							'message'            => sprintf( esc_html__( '%s variations created.', 'vg_sheet_editor' ), 0 ),
							'force_complete'     => true,
							'deleted'            => array(),
							'data'               => array(),
			...
			if ( empty( $product_ids ) ) {
				...
				wp_send_json_error( array( 'message' => esc_html__( 'Target products not found.', 'vg_sheet_editor' ) ) );
			}
			...
			wp_send_json_success(
				array(
					/* translators: %s: Number of variations created */
					'message'            => sprintf( esc_html__( '%s variations created.', 'vg_sheet_editor' ), $variations_count ),
					'deleted'            => array_unique( VGSE()->deleted_rows_ids ),
					'data'               => $data_rows,
		...
		function link_all_variations( $post_id ) {
			...
			if ( version_compare( WC()->version, '3.0' ) < 0 ) {
				return new WP_Error( 'wpse', array( 'message' => esc_html__( 'The option to create variations for every combination of attributes requires WooCommerce 3.0 or higher. Please update WooCommerce.', 'vg_sheet_editor' ) ) );
			}

			if ( ! WP_Sheet_Editor_Helpers::current_user_can( 'edit_products' ) ) {
				return new WP_Error( 'wpse', array( 'message' => esc_html__( 'User not allowed', 'vg_sheet_editor' ) ) );
			}

			if ( ! $post_id ) {
				return new WP_Error( 'wpse', array( 'message' => esc_html__( 'Data missing, try again later.', 'vg_sheet_editor' ) ) );
			}
			...
			if ( ! is_callable( array( $data_store, 'create_all_product_variations' ) ) ) {
				return new WP_Error( 'wpse', array( 'message' => esc_html__( 'Wrong product type. Make sure it is a variable product.', 'vg_sheet_editor' ) ) );
			}
		...
		function get_product_variations() {
			...
			if ( empty( $_REQUEST['product_id'] ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'Please select a source product.', 'vg_sheet_editor' ) ) );
			}
			$product_id = VGSE()->helpers->_get_post_id_from_search( sanitize_text_field( wp_unslash( $_REQUEST['product_id'] ) ) );
...
WHERE post_parent = %d AND post_type = %s AND post_status IN ('publish', 'draft')",
		...
		function update_default_attributes() {
			...
			if ( ! WP_Sheet_Editor_Helpers::current_user_can( 'edit_post', $post_id ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'You dont have enough permissions to view this page.', 'vg_sheet_editor' ) ) );
			}
		...
		function maybe_save_variations( $response, $data, $post_type, $spreadsheet_columns, $request ) {
			...
			foreach ( $formatted_for_api as $row_to_save ) {
				...
				if ( ! empty( $response_data['update'] ) ) {
					foreach ( $response_data['update'] as $variation_response ) {
						...
						/* translators: 1: ID, 2: Error message */
						$error_messages[] = sprintf( esc_html__( 'Error on row ID: %1$d - %2$s', 'vg_sheet_editor' ), $variation_response['id'], $variation_response['error']['message'] );
					}
				}
			...
			if ( ! empty( $error_messages ) ) {
				return new WP_Error( 'wpse', esc_html__( 'Please correct the error and save again.', 'vg_sheet_editor' ) . '<br>' . implode( '<br>', $error_messages ) );
			}
