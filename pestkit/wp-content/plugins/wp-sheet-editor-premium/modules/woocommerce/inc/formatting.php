	...
	class WPSE_WC_Products_Data_Formatting {
		...
		function _sync_product_lookup_table( $product_id, $modified_data = array() ) {
			...
			if ( ! $lookup_already_synced && $sync_required ) {
				...
				// We resave the a lookup column to force WC to execute the internal, protected method update_lookup_table()
				if ( VGSE()->get_option( 'use_simple_wc_lookup_update' ) === 'simple' ) {
					$this->update_lookup_table( $product->get_id(), 'wc_product_meta_lookup' );
					$this->clear_wc_caches( $product );
				} else {
					$is_virtual = wc_string_to_bool( get_post_meta( $product_id, '_virtual', true ) );
		...
		protected function use_cogs_lookup_column(): bool {
			$cogs_controller = wc_get_container()->get( \Automattic\WooCommerce\Internal\CostOfGoodsSold\CostOfGoodsSoldController::class );
		...
		public function prepare_sale_dates_for_display( $value, $post, $key, $column_settings ) {
			if ( is_numeric( $value ) ) {
				...
				return function_exists( 'wp_date' ) ? wp_date( 'Y-m-d', $value ) : date_i18n( 'Y-m-d', $value );
			}
			return $value;
		}
		...
		function product_updated_with_formula( $post_id, $initial_data, $modified_data, $column, $formula, $post_type, $cell_args, $spreadsheet_columns ) {
			...
			if ( in_array( $column, array( '_price', '_regular_price', '_sale_price', '_sale_price_dates_from', '_sale_price_dates_to' ), true ) ) {
				$this->_sync_price_meta( $post_id );
			}
		...
		function _sync_price_meta( $product_id ) {
			$product = wc_get_product( $product_id );
			...
			foreach ( $data as $item ) {
				if ( $update_parent ) {
					...
					if ( $post && $post->post_type === 'product_variation' && ! in_array( (int) $post->post_parent, $ids, true ) ) {
						...
						$this->product_updated_on_spreadsheet(
		...
		function product_updated_on_spreadsheet( $product_id, $item, $data, $post_type ) {
			...
			if ( array_intersect( array_keys( $item ), array( '_price', '_regular_price', '_sale_price', '_sale_price_dates_from', '_sale_price_dates_to' ) ) ) {
				$this->_sync_price_meta( $product_id );
			}
		...
		function _maybe_migrate_product_to_variation( $product_id, $row ) {
			...
			$variation_attributes = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $wpdb->postmeta WHERE post_id = %d AND meta_key LIKE %s AND meta_value <> ''", $product_id, 'attribute_%' ) );
