				...
				foreach ( $quick_actions as $bulk_action => $action ) {
					...
					$editor->args['toolbars']->register_item(
						...
						array(
							...
							'allow_in_frontend' => true,
							'parent'            => 'run_formula',
							'order'             => isset( $action['order'] ) ? $action['order'] : 10,
						),
						$post_type
					);
				}
		...
		function get_formulas_data() {
			...
			$formulas_data = array(
				...
				array(
					...
					array(
						...
						array(
							array(
								...
								'html_attrs' => array(
									'type' => 'text',
									'@blur' =>  'validateCustomFormula($el.value)',
								),
								/* translators: %s: documentation URL */
					...
					array(
						'label'                  => esc_html__( 'Import external images', 'vg_sheet_editor' ),
						'description'            => esc_html__( 'Download external images from img tags and replace them with local media library URLs.', 'vg_sheet_editor' ),
						'fields_relationship'    => 'AND',
				...
				array(
					'text'                   => array(
						...
						'custom'                          => 'default',
						'import_external_images'          => 'default',
					),
					'email'                  => array(
						'set_value'                => 'default',
						'replace'                  => 'default',
