...
if ( ! class_exists( 'WP_Sheet_Editor_Formulas' ) ) {
	...
	class WP_Sheet_Editor_Formulas {
			...
			if ( strpos( $formula, '=REPLACE(' ) !== false ) {
				if ( strpos( $formula, ',' ) !== false ) {
					...
					if ( trim( $result_original_formula[1][0] ) === '$current_value$' ) {
						...
						if ( empty( $result[1][0] ) && ! is_string( $result[1][0] ) ) {
							$result[1][0] = '';
						}
						if ( empty( $result[2][0] ) && ! is_string( $result[2][0] ) ) {
							$result[2][0] = '';
						}

						if ( strpos( $result[1][0], self::$regex_flag ) === 0 ) {
							$sanitized_search = array(
								array(
					...
					return new WP_Error( VGSE()->options_key, esc_html__( 'Invalid #898AJSI. The replace requires 2 parameters, we received one', 'vg_sheet_editor' ) );
				}
				...
				if ( empty( $result[1] ) ) {
					return new WP_Error( VGSE()->options_key, esc_html__( 'Invalid #8W89PQ. Math formula is empty.', 'vg_sheet_editor' ) );
				}
				$raw_formula = $result[1];

				if ( strpos( $raw_formula, ',' ) !== false ) {
					/* translators: %s: Math formula containing a comma */
					return new WP_Error( VGSE()->options_key, sprintf( esc_html__( 'Invalid #PQ8SPQ. The math formula contains a comma: %s', 'vg_sheet_editor' ), $formula ) );
				}
				$formula = preg_replace( '/[^0-9\.\+\*\-\/\(\)]/', '', $raw_formula );

				if ( empty( $formula ) && ! empty( $raw_formula ) || ! preg_match( '/\d/', $formula ) ) {
					return new WP_Error( VGSE()->options_key, esc_html__( 'Invalid #8W89PWQ. Math formula is invalid.', 'vg_sheet_editor' ) );
				}
				...
				if ( ! function_exists( $php_formula ) && ! is_callable( $php_formula ) ) {
					/* translators: %s: PHP function name that does not exist */
					return new WP_Error( VGSE()->options_key, sprintf( esc_html__( 'Invalid #PQ8PHP. The PHP function does not exist: %s', 'vg_sheet_editor' ), $php_formula ) );
				} else {
					$out['set1'] = $php_formula;
				}
			} else {
				return new WP_Error( VGSE()->options_key, esc_html__( 'Invalid #8W23CV. We accept MATH and REPLACE formulas only, we received an unknown formula type.', 'vg_sheet_editor' ) );
			}
		...
		public function get_sequential_number() {
			$current_sequence = (int) get_transient( 'vgse_sequence' . $this->current_bulk_edit_id );
			++$current_sequence;
			set_transient( 'vgse_sequence' . $this->current_bulk_edit_id, $current_sequence, WEEK_IN_SECONDS );
		...
		public function _replace_placeholders( $formula, $data, $post_id, $post_type, $cell_args ) {
			...
			$formula = str_replace( '$random_number$', wp_rand( 10000, 999999 ), $formula );
			...
			if ( strpos( $formula, '$sequential_number$' ) !== false ) {
				$current_sequence = (int) $this->get_sequential_number();
			...
			if ( strpos( $formula, '$random_value_start$' ) !== false && strpos( $formula, '$random_value_end$' ) !== false ) {
				...
				if ( ! $raw_values_list ) {
					return new WP_Error( VGSE()->options_key, esc_html__( 'The random values could not be read.', 'vg_sheet_editor' ) );
				}
		...
		public function apply_formula_to_data( $formula, $data, $post_id, $cell_args, $post_type ) {
			...
			if ( $prepared_formula['type'] === 'REPLACE' ) {
				...
				if ( ! is_numeric( $data ) ) {
					/* translators: %1$s: Existing data that is not numeric, %2$d: Post ID */
					return new WP_Error( VGSE()->options_key, sprintf( esc_html__( 'The math formula can\'t be applied. We found some existing data is not numeric. Data found: %1$s, ID: %2$d', 'vg_sheet_editor' ), $data, $post_id ) );
				}
				...
				if ( $data === $formula ) {
					/* translators: %1$s: Math formula that could not be executed, %2$d: Post ID */
					return new WP_Error( VGSE()->options_key, sprintf( esc_html__( 'Error. The math engine could not execute the math operation: %1$s, ID: %2$d', 'vg_sheet_editor' ), $formula, $post_id ) );
				}
		...
		public function can_execute_formula_as_sql( $formula, $column, $post_type, $spreadsheet_columns, $raw_form_data ) {
			...
			if ( in_array( $raw_form_data['action_name'], array( 'add_time', 'reduce_time', 'remove_duplicates', 'remove_duplicates_title_content', 'remove_everything_after', 'remove_everything_before', 'send_email', 'learndash_enroll_course', 'import_external_images' ), true ) ) {
				return false;
			}
			...
			if ( in_array( $column['key'], array( 'wpse_status', 'post_status' ), true ) && ( in_array( 'delete', $raw_form_data['formula_data'], true ) || $raw_form_data['action_name'] === 'delete_user' ) ) {
				return false;
			}
		...
		public function execute_formula_as_sql( $post_ids, $formula, $column, $post_type ) {
			...
			if ( strpos( $prepared_formula['set1'], '$current_value$' ) !== false && trim( $prepared_formula['set1'] ) !== '$current_value$' ) {
				return new WP_Error( VGSE()->options_key, esc_html__( 'The search parameter must contain $current_value$ only', 'vg_sheet_editor' ) );
			}
			...
			if ( $prepared_formula['set1'] === '$current_value$' ) {
				...
				if ( $set2_prepared === false ) {
					return new WP_Error( VGSE()->options_key, esc_html__( 'Value in the replace section is not valid', 'vg_sheet_editor' ) );
				}
			...
			if ( ! VGSE()->get_option( 'disable_auto_purge_post_cache' ) && VGSE()->helpers->get_current_provider()->is_post_type ) {
				foreach ( $post_ids as $post_id ) {
					VGSE()->helpers->maybe_auto_purge_post_cache( $post_id );
				}
			}
		...
		public function bulk_execute_formula_ajax() {
			...
			$settings = array(
				'column'        => sanitize_text_field( wp_unslash( $_REQUEST['column'] ) ),
				...
				'cells'         => isset( $_REQUEST['cells'] ) && is_array( $_REQUEST['cells'] ) ? wp_unslash( $_REQUEST['cells'] ) : array(),
				...
				'raw_form_data' => array(
					...
					'formula_data'          => isset( $raw_form_data['formula_data'] ) ? array_map( 'wp_kses_post', $raw_form_data['formula_data'] ) : array(),
					'action_name'           => sanitize_text_field( $raw_form_data['action_name'] ),
					'use_slower_execution'  => ! empty( $raw_form_data['use_slower_execution'] ),
					'is_preview'            => ! empty( $_REQUEST['is_preview'] ),
					'bulk-edit-select-rows' => isset( $raw_form_data['bulk-edit-select-rows'] ) ? sanitize_text_field( $raw_form_data['bulk-edit-select-rows'] ) : '',
				),
			);

			$settings = apply_filters( 'vg_sheet_editor/formulas/bulk_execute_formula_ajax_settings', $settings );
		...
		public function validation_before_formula( $request_data = array() ) {
			...
			if ( ! VGSE()->helpers->user_can_delete_post_type( $post_type ) && ( ( $action_name === 'set_value' && in_array( 'delete', $raw_form_data['formula_data'], true ) ) || $action_name === 'delete_user' ) ) {
				return new WP_Error( 'vgse', esc_html__( 'You do not have permission to delete posts.', 'vg_sheet_editor' ) );
			}
			...
			if ( in_array( $action_name, array( 'add_time', 'reduce_time' ), true ) ) {
				if ( empty( $raw_form_data['formula_data'] ) || empty( $raw_form_data['formula_data'][0] ) || empty( $raw_form_data['formula_data'][1] ) ) {
					return new WP_Error( 'vgse', esc_html__( 'Missing number and time unit.', 'vg_sheet_editor' ) );
				}
				$seconds_constant = strtoupper( $raw_form_data['formula_data'][1] ) . '_IN_SECONDS';
				if ( ! defined( $seconds_constant ) ) {
					return new WP_Error( 'vgse', esc_html__( 'Invalid time unit.', 'vg_sheet_editor' ) );
				}
			}

			if ( in_array( $action_name, array( 'remove_terms' ), true ) ) {
				if ( empty( $raw_form_data['formula_data'] ) || empty( $raw_form_data['formula_data'][0] ) ) {
					return new WP_Error( 'vgse', esc_html__( 'Please indicate which terms you want to remove.', 'vg_sheet_editor' ) );
				}
			}

			if ( $action_name === 'send_email' && ! WP_Sheet_Editor_Helpers::current_user_can( 'manage_options' ) ) {
				return new WP_Error( 'vgse', esc_html__( 'Only administrators are allowed to send emails to users.', 'vg_sheet_editor' ) );
			}

			if ( $action_name === 'send_email' && ( empty( $raw_form_data['formula_data'][0] ) || empty( $raw_form_data['formula_data'][1] ) ) ) {
				return new WP_Error( 'vgse', esc_html__( 'Please enter the email subject and email message.', 'vg_sheet_editor' ) );
			}

			if ( ! isset( $spreadsheet_columns[ $column ] ) ) {
				return new WP_Error( 'vgse', esc_html__( 'The column selected is not valid.', 'vg_sheet_editor' ) );
			}
			...
			if ( in_array( $raw_form_data['action_name'], array( 'remove_duplicates', 'remove_duplicates_title_content' ), true ) && ! isset( $spreadsheet_columns['post_status'] ) ) {
				return new WP_Error( 'vgse', esc_html__( 'Error. The status column must be enabled in the spreadsheet before we can remove duplicates.', 'vg_sheet_editor' ) );
			}

			if ( ! empty( $request_data['is_preview'] ) && ! $this->is_preview_allowed( $request_data ) ) {
				return new WP_Error( 'vgse', esc_html__( 'The preview mode does not support this action.', 'vg_sheet_editor' ) );
			}
		...
		public function get_query( $request_data ) {
			...
			// If we're deleting users, always use page 1 because the
			// pagination breaks when the rows are deleted and the real page number becomes wrong
			if ( $post_type === 'user' && $raw_form_data['action_name'] === 'delete_user' ) {
				$page = 1;
			}
			...
			$query = apply_filters( 'vg_sheet_editor/formulas/execute/query_results', $query, $base_query, $request_data );
		...
		public function handle_bulk_edit_finished( $formula, $bulk_edit_id, $editor, $post_type ) {
			...
			if ( file_exists( $email_sent_log_path ) ) {
				wp_delete_file( $email_sent_log_path );
			}

			delete_option( $bulk_edit_id . '_total' );
		}
		public function handle_empty_query_results( $page, $formula, $bulk_edit_id, $post_type, $editor ) {
			if ( $page === 1 ) {
				...
				return array(
					'force_complete' => true,
					'message'        => '<p>' . __( 'Complete.', 'vg_sheet_editor' ) . '</p>',
				);
			}
		}
		public function bulk_execute_formula_on_plain_values( $request_data ) {
			$formula             = wp_unslash( $request_data['formula'] );
			$column_key          = $request_data['column'];
			$post_type           = $request_data['post_type'];
			$spreadsheet_columns = VGSE()->helpers->get_unfiltered_provider_columns( $post_type );
			$column_settings     = $spreadsheet_columns[ $column_key ];

			if ( empty( $request_data['cells'] ) || ! is_array( $request_data['cells'] ) ) {
				return new WP_Error( 'vgse', __( 'No cells data received.', 'vg_sheet_editor' ) );
			}

			$modified_cells = array();
			foreach ( $request_data['cells'] as $cell_data ) {
				// Sanitize received data
				$row     = isset( $cell_data['row'] ) ? (int) $cell_data['row'] : null;
				$col     = isset( $cell_data['col'] ) ? (int) $cell_data['col'] : null;
				$value   = isset( $cell_data['value'] ) ? $cell_data['value'] : ''; // Value is not sanitized to allow html, it will be sanitized by the formula parser
				$post_id = isset( $cell_data['post_id'] ) ? (int) $cell_data['post_id'] : 0;

				if ( is_null( $row ) || is_null( $col ) ) {
					continue;
				}

				$modified_value = $this->apply_formula_to_data( $formula, $value, $post_id, $column_settings, $post_type );

				if ( is_wp_error( $modified_value ) ) {
			...
			$total_cells = count( $modified_cells );
			$message     = sprintf( __( '<p class="success-message" data-column-key="%s">Cells to process: {total}, Progress: {progress_percentage}%%, We have updated {edited} cells.</p>', 'vg_sheet_editor' ), esc_attr( $request_data['column'] ), $total_cells, $total_cells, $total_cells );
		...
		public function bulk_execute_formula( $request_data = array() ) {
			...
			$rows_selection_type = isset( $request_data['raw_form_data']['bulk-edit-select-rows'] ) ? $request_data['raw_form_data']['bulk-edit-select-rows'] : '';
			if ( 'edit_plain_values' === $rows_selection_type ) {
				return $this->bulk_execute_formula_on_plain_values( $request_data );
			}
			...
			$bulk_edit_total = (int) get_option( $bulk_edit_id . '_total' );
			if ( $query->found_posts && ! $bulk_edit_total ) {
				$bulk_edit_total = $query->found_posts;
				update_option( $bulk_edit_id . '_total', $bulk_edit_total, false );
			}
			$total = $bulk_edit_total;
			...
			// Remove list flag if this is not a bulk deletion, if it's a bulk deletion we'll let the client send more requests and mark it as finished when no posts are found
		...
		private function _add_or_reduce_time( $raw_form_data, &$data ) {
			$is_sequential    = ! empty( $raw_form_data['formula_data'][2] ) && $raw_form_data['formula_data'][2] === 'yes';
			$timestamp        = ( empty( $data ) ) ? current_time( 'timestamp' ) : strtotime( $data );
			$seconds_constant = strtoupper( $raw_form_data['formula_data'][1] ) . '_IN_SECONDS';
			$units            = (int) $raw_form_data['formula_data'][0];
			if ( $is_sequential ) {
				$units += $this->get_sequential_number() - 1;
			}
			$seconds_for_edit = constant( $seconds_constant ) * $units;
		...
		private function _import_external_images( $post_id, &$data ) {
			if ( empty( $data ) || ! is_string( $data ) ) {
				return;
			}

			// Extract all remote image URLs from the data
			preg_match_all( '/(https?:\/\/[^\s"\'><\(\)\{\}\[\]\\\\\/]+(?:\/[^\s"\'><\(\)\{\}\[\]\\\\\/]+)*\.(?:jpe?g|png|gif|webp|svg|bmp|ico)\b(?:\?[^\s"\'><\(\)\{\}\[\]\\\\]*)?)(?=[\s"\'><\(\)\{\}\[\]\\\\,;]|$)/i', $data, $matches );

			if ( ! empty( $matches[1] ) ) {
				$urls_to_replace         = array();
				$wpcontent_without_proto = preg_replace( '/^https?:\/\//', '', WP_CONTENT_URL );

				foreach ( $matches[1] as $url ) {
					// Check if URL starts with http:// or https:// (since we only import external remote URLs)
					if ( strpos( $url, 'http://' ) === 0 || strpos( $url, 'https://' ) === 0 ) {
						$url_without_proto = preg_replace( '/^https?:\/\//', '', $url );
						if ( strpos( $url_without_proto, $wpcontent_without_proto ) !== 0 ) {
								$urls_to_replace[] = $url;
						}
					}
				}
				...
				if ( ! empty( $urls_to_replace ) ) {
					// Remove duplicates to avoid importing the same image multiple times per cell
					$urls_to_replace = array_unique( $urls_to_replace );

					// Sort by length descending to replace longer URLs first (avoid prefix collision)
					usort( $urls_to_replace, function( $a, $b ) {
						return strlen( $b ) - strlen( $a );
					} );

					foreach ( $urls_to_replace as $external_url ) {
						$ids = VGSE()->helpers->maybe_replace_urls_with_file_ids( array( $external_url ), $post_id );
						if ( ! empty( $ids ) ) {
								$media_id = current( $ids );
							if ( $media_id ) {
								$new_url = wp_get_attachment_url( $media_id );
								if ( $new_url ) {
												$data = str_replace( $external_url, $new_url, $data );
								}
							}
						}
					}
		...
		*/
		public function apply_formula_to_row( $post, $column_settings, $formula, $post_type, $spreadsheet_columns, $raw_form_data, $editor, &$editions_count, &$updated_ids, &$processed_ids, &$count, $email_sent_log_path, $column, $request_data, $is_preview ) {
			...
			} elseif ( $raw_form_data['action_name'] === 'import_external_images' ) {
				$this->_import_external_images( $post_id, $data );
			} else {
			...
			if ( ! VGSE()->get_option( 'disable_auto_purge_post_cache' ) && VGSE()->helpers->get_current_provider()->is_post_type ) {
				VGSE()->helpers->maybe_auto_purge_post_cache( $post->ID );
			}
		...
		*
		* @return void|WP_Error Null if data was saved successfully, WP_Error otherwise.
		*/
