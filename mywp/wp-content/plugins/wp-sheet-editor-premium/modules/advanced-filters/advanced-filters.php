		...
		function add_advanced_taxonomy_query( $clauses, $wp_query ) {
			...
			foreach ( $post_data_query as $post_data_parameters ) {
				...
				if ( in_array( $post_data_parameters['compare'], array( 'LIKE', 'NOT LIKE' ) ) ) {
					$post_data_parameters['value'] = '%' . htmlspecialchars( $post_data_parameters['value'], ENT_QUOTES ) . '%';
				}
				if ( in_array( $post_data_parameters['compare'], array( '=', '!=' ) ) ) {
					$post_data_parameters['value'] = htmlspecialchars( $post_data_parameters['value'], ENT_QUOTES );
				}
...
WHERE ml.sku IN ($placeholders) AND p.post_type = 'product_variation'",
				...
				if ( class_exists( 'WooCommerce' ) && in_array( $meta_query['key'], array( '_sale_price_dates_from', '_sale_price_dates_to' ), true ) && ! empty( $meta_query['value'] ) ) {
					...
					$meta_query_args[ $index ]['value'] = function_exists( 'wp_date' ) ? wp_date( 'U', get_gmt_from_date( $meta_query['value'], 'U' ) ) : date_i18n( 'U', get_gmt_from_date( $meta_query['value'], 'U' ) );
				}
			...
			<div class="fields-wrap" :data-advanced-filter-index="index">
				...
				<div class="field-wrap search-value-wrap" x-show="!isValueFieldHidden(filter.compare, $el)">
					...
					<template x-if="getAdvancedFilterValueType(filter) === 'text'">
						<input :name="'meta_query['+index+'][value]'" type="text" class="wpse-advanced-filters-value-selector" x-model.lazy="filter.value" @keyup.enter.prevent="jQuery('.run_filters-container .toolbar-submenu').is(':visible') ? startQuickSearch : runSearch" />
					</template>
					<template x-if="getAdvancedFilterValueType(filter) === 'number'">
						<input :name="'meta_query['+index+'][value]'" step="0.01" type="number" class="wpse-advanced-filters-value-selector" x-model.lazy="filter.value"  @keyup.enter.prevent="jQuery('.run_filters-container .toolbar-submenu').is(':visible') ? startQuickSearch : runSearch"/>
					</template>
