...
<p><?php esc_html_e( 'The exported file does not exist. This could happen for the following reasons:', 'vg_sheet_editor' ); ?>
</p>
<ol>
    <li>
        <?php
					/* translators: %s: File expiration hours */
					printf( esc_html__( 'If you are trying to download something that was exported several hours ago, the file might have expired. Files expire after %s hours.', 'vg_sheet_editor' ), esc_html( $this->file_expiration_hours() ) );
				?>
    </li>
    <li>
        <?php
					/* translators: %s: Exports directory path */
					printf( esc_html__( 'Maybe your server did not allow us to create the file. It happens if your server is not configured correctly and our folder %s does not have write permissions, so we\'re not able to create the file. You should ask your hosting provider to fix the folder permissions.', 'vg_sheet_editor' ), esc_html( str_replace( wp_normalize_path( ABSPATH ), '/', $this->exports_dir ) ) );
				?>
    </li>
</ol>
<?php
		...
		function import_data( $settings ) {
			...
			if ( in_array( $settings['import_type'], array( 'xml' ), true ) ) {
				$xml_file                = $this->imports_dir . $settings['import_file'];
				$settings['import_file'] = basename( $this->convert_xml_to_csv( $xml_file ) );
				$settings['import_type'] = 'csv';
			}
		...
		function upload_data_for_import() {
			...
			if ( $data_type === 'url' ) {
				...
				if ( ! in_array( $file_type, $allowed_input_type ) ) {
					wp_send_json_error( array( 'message' => esc_html__( 'Wrong file extension. We accept CSV, JSON, XLSX, XLS, XML only', 'vg_sheet_editor' ) ) );
				}
			...
			if ( in_array( $file_type, array( 'xlsx', 'xls' ), true ) ) {
				...
				$file_path = $this->convert_xml_to_csv( $file_path );
		...
		/**
		 * Convert XML file to CSV with deep nesting support.
		 *
		 ...
		 */
		function convert_xml_to_csv( $file_path ) {
			if ( ! file_exists( $file_path ) ) {
				return new WP_Error( 'xml_error', __( 'File not found.', 'vg_sheet_editor' ) );
			}

			libxml_use_internal_errors( true );
			$xml = simplexml_load_file( $file_path );
			if ( false === $xml ) {
				return new WP_Error( 'xml_error', __( 'Invalid XML file.', 'vg_sheet_editor' ) );
			}

			$rows = array();

			/**
			 * In a WordPress WXR file, the actual content is inside <channel> -> <item>.
			 * If the XML structure varies, we look for the first level that contains multiple children.
			 */
			$items = $xml->channel->item ?? $xml->children();

			foreach ( $items as $item ) {
				$flat_row = array();
				$this->flatten_node( $item, $flat_row );
				$rows[] = $flat_row;
			}

			if ( empty( $rows ) ) {
				return new WP_Error( 'xml_error', __( 'No data found in XML.', 'vg_sheet_editor' ) );
			}
			...
			$csv_file_path = preg_replace( '/\.xml$/i', '.csv', $file_path );

			// Pass the rows and headers to your existing CSV generator
			$this->_array_to_csv( $rows, $csv_file_path, $this->_str_putcsv( $headers ) );

			return $csv_file_path;
		}

		/**
		 * Recursively flattens an XML node into a single-level associative array.
		 */
		private function flatten_node( $node, &$result, $prefix = '' ) {
			$children = $node->children( null, true ); // Get children from all namespaces

			if ( count( $children ) === 0 ) {
				// Base case: No children, just the value
				$result[ rtrim( $prefix, '.' ) ] = (string) $node;
				return;
			}
			...
			foreach ( $children as $name => $child ) {
				...
				// Handle repeated keys (like multiple <wp:postmeta> tags)
				// We append an index if the key already exists in this row
				$final_key = $key;
				$i         = 1;
				while ( isset( $result[ $final_key ] ) ) {
					$final_key = $key . '_' . $i;
					++$i;
				}

				if ( count( $child->children( null, true ) ) > 0 ) {
					$this->flatten_node( $child, $result, $final_key . '.' );
				} else {
					$result[ $final_key ] = (string) $child;
				}
			}
		}
		...
		function register_export_data_fields( $settings ) {
			...
			$settings['line_items_separate_rows'] = isset( $_REQUEST['line_items_separate_rows'] ) ? VGSE()->helpers->do_booleable( $_REQUEST['line_items_separate_rows'] ) : false;
		...
		function export_csv( $out, $wp_query_args, $spreadsheet_columns, $clean_data ) {
			...
			$allowed_column_keys_filled = array_fill_keys( $allowed_column_keys, '' );
			$required_column_keys_map   = array();
			foreach ( $required_column_keys as $k ) {
				$required_column_keys_map[ $k ] = true;
			}

			foreach ( $cleaned_rows as $row_index => $row ) {

				// Remove all fields from the row that are not found in the list of fields to export
				$row = array_intersect_key( $row, $allowed_column_keys_filled );

				foreach ( $row as $column_key => $value ) {
					$required_column_keys_map[ $column_key ] = true;

					// Convert line breaks to p and br tags before we strip the line breaks
					if ( isset( $spreadsheet_columns[ $column_key ] ) && in_array( $spreadsheet_columns[ $column_key ]['type'], array( 'boton_tiny' ) ) ) {
						$value = wpautop( $value );
					}
			...
			$required_column_keys = array_keys( $required_column_keys_map );
			...
			if ( ( $wp_query_args['posts_per_page'] * $wp_query_args['paged'] ) >= $out['total'] ) {
				$file_type = 'csv';
				// If the excel sync plugin exists, the sync will handle the creation of the Excel file in OneDrive, no need to convert here
				if ( 'excel' === $clean_data['target_software'] && ! class_exists( 'WPSE_Excel' ) ) {
