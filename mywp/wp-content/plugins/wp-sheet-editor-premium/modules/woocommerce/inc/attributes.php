...
if ( ! class_exists( 'WP_Sheet_Editor_WooCommerce_Attrs' ) ) {
	...
	class WP_Sheet_Editor_WooCommerce_Attrs {
		...
		public $post_type_variation = 'product_variation';
		public $cell_key            = '_vgse_create_attribute';
		...
		function remove_attribute_labels_from_search( $labels, $post_type ) {
			if ( $post_type === $this->post_type ) {
				...
				$final_labels[] = esc_html__( 'Product attributes', 'vg_sheet_editor' );
				$labels         = $final_labels;
			}
		...
		function execute_formula_toggle_attribute_settings( $update_count, $raw_form_data, $post_ids, $column_settings ) {
			...
			// We must use sanitize_title so the attribute slugs in Hebrew match the db values
			$attribute      = sanitize_title( $raw_form_data['formula_data'][2] );
		...
		function execute_formula_default_attribute_replace( $update_count, $raw_form_data, $post_ids, $column_settings ) {
			...
			// Find products that have the specific attribute in their _product_attributes
			$product_attribute_rows = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT post_id, meta_value FROM $wpdb->postmeta 
				WHERE post_id IN ($ids_in_query_placeholders) 
				AND meta_key = '_product_attributes' 
				AND meta_value LIKE %s",
					array_merge( $post_ids, array( '%"' . $wpdb->esc_like( $attribute_name ) . '"%' ) )
				),
				ARRAY_A
			);

			$products_with_attribute = array();
			foreach ( $product_attribute_rows as $row ) {
				$attributes = maybe_unserialize( $row['meta_value'] );
				if ( is_array( $attributes ) && isset( $attributes[ $attribute_name ] ) ) {
					$products_with_attribute[] = $row['post_id'];
				}
			}

			if ( empty( $products_with_attribute ) ) {
				return 0;
			}

			// Now check default attributes for products that have this attribute
			$default_ids_placeholders = implode( ', ', array_fill( 0, count( $products_with_attribute ), '%d' ) );
			$default_attribute_rows   = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT post_id, meta_value FROM $wpdb->postmeta 
				WHERE post_id IN ($default_ids_placeholders) 
				AND meta_key = '_default_attributes'",
					$products_with_attribute
				),
				ARRAY_A
			);

			// Create a map of products with default attributes
			$products_with_defaults = array();
			foreach ( $default_attribute_rows as $row ) {
				$products_with_defaults[ $row['post_id'] ] = $row;
			}

			$update_count = 0;

			foreach ( $products_with_attribute as $product_id ) {
				$updated = false;

				if ( isset( $products_with_defaults[ $product_id ] ) ) {
					// Product has default attributes meta
					$row        = $products_with_defaults[ $product_id ];
					$attributes = maybe_unserialize( $row['meta_value'] );
					...
					if ( empty( $search ) ) {
						// If search is empty, set default for products without this attribute set
						...
						if ( isset( $attributes[ $attribute_name ] ) && $attributes[ $attribute_name ] === $search ) {
							if ( $replace ) {
								$attributes[ $attribute_name ] = $replace;
							} else {
					...
					if ( $updated ) {
						update_post_meta( $product_id, '_default_attributes', $attributes );
						++$update_count;
					}
				} else {
					// Product doesn't have default attributes meta yet
					if ( empty( $search ) && ! empty( $replace ) ) {
						// Create new default attributes with our attribute
						$attributes = array( $attribute_name => $replace );
						update_post_meta( $product_id, '_default_attributes', $attributes );
						++$update_count;
					}
				}
		...
		function add_formula_type_toggle_attribute_settings( $formulas, $post_type ) {
			...
			$formulas['default_actions']['wc_attributes_toggle_setting']         = array(
				'label'               => esc_html__( 'Change attribute settings', 'vg_sheet_editor' ),
				'description'         => '',
				...
				array(
					array(
						'label'   => esc_html__( 'What setting do you want to change?', 'vg_sheet_editor' ),
						'tag'     => 'select',
						'options' => '<option value="">--</option><option value="is_visible">' . esc_html__( 'Is Visible?', 'vg_sheet_editor' ) . '</option>' . '<option value="is_variation">' . esc_html__( 'Used for Variations?', 'vg_sheet_editor' ) . '</option>',
					),
					array(
						'label'   => esc_html__( 'New value', 'vg_sheet_editor' ),
						'tag'     => 'select',
						'options' => '<option value="0">' . esc_html__( 'No', 'vg_sheet_editor' ) . '</option>' . '<option value="1">' . esc_html__( 'Yes', 'vg_sheet_editor' ) . '</option>',
					),
					array(
						'label'   => esc_html__( 'What attribute do you want to edit?', 'vg_sheet_editor' ),
						'tag'     => 'select',
			...
			$formulas['default_actions']['wc_default_attribute_replace']         = array(
				'label'               => esc_html__( 'Replace a default attribute term', 'vg_sheet_editor' ),
				'description'         => '',
				'fields_relationship' => 'AND',
				'jsCallback'          => 'vgseGenerateReplaceFormula',
				...
				array(
					array(
						'label'   => esc_html__( 'Enter the attribute name', 'vg_sheet_editor' ),
						'tooltip' => esc_html__( 'If this is a global attribute, enter the attribute key (i.e. pa_color). Enter the name if this is a custom attribute.', 'vg_sheet_editor' ),
						'tag'     => 'input',
					),
					array(
						'label'   => esc_html__( 'Replace this term', 'vg_sheet_editor' ),
						'tag'     => 'input',
						'tooltip' => esc_html__( 'Enter the term name or slug.', 'vg_sheet_editor' ),
					),
					array(
						'label'   => esc_html__( 'With this term', 'vg_sheet_editor' ),
						'tag'     => 'input',
						'tooltip' => esc_html__( 'Leave this empty if you want to unset the term.', 'vg_sheet_editor' ),
					),
				),
			);
		...
		function replace_all_terms_with_real_terms( $item, $post_id, $post_type, $spreadsheet_columns ) {
			...
			$select_all_keyword = esc_html__( 'Select all', 'vg_sheet_editor' );
		...
		function add_select_all_option_to_terms( $terms, $taxonomy, $source ) {
			...
			if ( strpos( $taxonomy, 'pa_' ) !== false && $source === 'taxonomy_column' ) {
				$terms = array_merge( array( esc_html__( 'Select all', 'vg_sheet_editor' ) ), $terms );
			}
			return $terms;
		}
		...
		function register_columns( $editor ) {
			...
			foreach ( $all_attributes['custom'] as $attribute_key => $attribute_label ) {
				...
				$editor->args['columns']->register_item(
					...
					array(
						...
						'title'                 => sprintf( esc_html__( 'Custom attribute: %s', 'vg_sheet_editor' ), esc_html( $attribute_label ) ),
						'type'                  => '',
						...
						'formatted'             => array(
							'comment' => array( 'value' => esc_html__( 'Enter multiple attributes separated by |', 'vg_sheet_editor' ) ),
						),
			...
			$editor->args['columns']->register_item(
				...
				array(
					...
					'column_width'               => 150,
					'title'                      => esc_html__( 'Product attributes', 'vg_sheet_editor' ),
					'supports_formulas'          => true,
					...
					'prefetch_meta_key'          => '_product_attributes',
					'formatted'                  => array(
						'data'     => '_vgse_create_attribute',
						'renderer' => 'html',
						'readOnly' => true,
					...
					'allow_to_rename'            => true,
					'type'                       => 'handsontable',
					'edit_button_label'          => esc_html__( 'Edit attributes', 'vg_sheet_editor' ),
					'edit_modal_id'              => 'vgse-edit-attributes',
					'edit_modal_title'           => esc_html__( 'Edit attributes', 'vg_sheet_editor' ),
					// translators: 1: term delimiter, 2: attributes page url
					...
					'handsontable_column_names'  => array(
						$this->post_type    => array( esc_html__( 'Name', 'vg_sheet_editor' ), esc_html__( 'Value', 'vg_sheet_editor' ), esc_html__( 'Position', 'vg_sheet_editor' ), esc_html__( 'Is visible?', 'vg_sheet_editor' ), esc_html__( 'Used for variation?', 'vg_sheet_editor' ) ),
						'product_variation' => array( esc_html__( 'Name', 'vg_sheet_editor' ), esc_html__( 'Value', 'vg_sheet_editor' ) ),
					),
